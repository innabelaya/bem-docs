...
Пакет предоставляет набор ENB-технологий для сборки файлов, обеспечивающих мультиязыковую поддержку БЭМ-проектов. Под мультиязыковой поддержкой понимается интернационализация (далее по тектсу также i18n).

С помощью технологий пакета `enb-bem-i18n` собирается модуль, который осуществляет интернационализацию (`i18n`) проекта.
...
* [keysets-xml](/api.ru.md#keysets-xml) — технология для локализации сервисов, использующих шаблоны XSLT.
...
>Особенности реализации для каждой библиотеки описаны в разделе [Ядро `i18n`](#Ядро-i18n).
...
```
при паблише  - только для разработки, но не для раельного испл
...
Файлы `all.js` и `<block-name>.i18n.js` могут содержать [ядро `i18n`](#Ядро-i18n) библиотеки `bem-bl` и `bem-core`, соответственно.
...
common.blocks/
    block1/
        ...
        block1.i18n/         # Директория для хранения файлов с переводами.
            en.js
            ru.js
            all.js           # Исходный файл с общими переводами (для
...
Ядро `i18n` — это библиотека для интернационализации, которая используется технологиями пакета `enb-bem-i18n`. Ядро находится в keysets-файлах (`<block-name>.i18n.js` или `<block-name>.all.js`) в одной из базовых библиотек блоков.

Библиотека [инициализируется](#Инициализация) («наполняется данными») с помощью keysets-файлов. Переводы (данные из библиотеки) становятся доступными благодаря [функции `i18n`](#Обработка-данных), которая принимает ключ и отдает значение (строку) для конкретного языка.
...
**Важно!** Чтобы включить ядро `i18n`, необходимо добавить `mustDeps`-зависимость блокам, которые используют i18n.
...
В результате работы технологий формируется `lang.<lang>.js`-файлы, содержащие строки переводов, соответствующие запрошенным ключам.
...
Cпособы использования `i18n` в JavaScript зависят от наличия в проекте модульной системы и ее типа. Файлы можно подключать как [в Node.js](#Использование-в-node.js), так и [в браузере](#Использование-в-браузере), независимо от используемой библиотеки (`bem-bl` или `bem-core`).

#### Использование в Node.js

Скомпилированный файл подключите как модуль в формате CommonJS.
...
* **В браузере без YModules как `BEM.I18N`**
  ...
  ```js
  BEM.I18N('scope', 'key'); // Ядро `i18n` предоставляется в глобальную переменную `BEM.I18N`.
  ```
