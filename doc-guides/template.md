...
>*Пример текста:* Вспомогательный блок. Используется в составе блока [имя-блока](#).
...
| -------- | --- | --------------------- | -------- |
| [Название параметра](#params-name1) (<i>опциональные аргументы указываются в квадратных скобках</i>)| Тип данных | `Значение по умолчанию` | Краткое описание. |
...
>В описание не включается изменение внешнего вида блока, связанное с применением модификатора темы.

>Раздел содержит подробное описание все сущностей блока в порядке, определенном в [обзоре блока](#block-overview). Каждая сущность (модификатор, элемент, поле, свойство, метод и т.д.) оформляется отдельным вложенным разделом.
