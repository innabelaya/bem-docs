...
|---------| -------- | --- | -------- |
| Название элемента, к которому относятся описываемые свойства и методы | <a href="#fields-name">Название свойства</a> (<i>якорь на соответствующий заголовок</i>) | <code>Тип данных</code> | Краткое описание. |
| Название элемента, к которому относятся описываемые свойства и методы | <a href="#fields-name">methodName</a>(<br><code>{String} arg1</code>, <br><code>{Array} arg2</code>, <br><code>[{Boolean} argN]</code>) (<i>опциональные аргументы указываются в квадратных скобках</i>)| <code>Возвращаемое значение</code> | Краткое описание. |
...
| -------- | --- | --------------------- | -------- |
| <a href="#params-name1">Название параметра</a> (<i>якорь на соответствующий заголовок</i>) | <code>Тип данных</code> | <code>Значение по умолчанию</code> | Краткое описание. |
| <a href="#params-name2">[Название параметра]</a> (<i>якорь на соответствующий заголовок</i>)| <code>Тип данных</code> | <code>Значение по умолчанию</code> | Краткое описание. |
...
>*Обратите внимание, строчные значения заключаются в одинарные вертикальные кавычки и бэктики.*

Значение по умолчанию: *Если отсутствует, ставится прочерк*. Обязательный модификатор. *Для необязательных модификаторов признак не указывается*.
...
Тип: `String`, `Number`, `Array`, `Boolean`. *Выберите нужный тип данных. Типы разделяются знаком `|`.*

Значение по умолчанию: *Если отсутствует, ставится прочерк*. Обязательное поле. *Для необязательных полей признак не указывается*.
...
Значение по умолчанию: *Если отсутствует, ставится прочерк*. Обязательный параметр. *Для необязательных параметров признак не указывается*.
